...
export async function POST(request) {
  try {
    ...
    if (existingCompany) {
      return NextResponse.json(
        { success: false, message: 'Es existiert bereits ein Firmenprofil für diesen Benutzer' },
        { status: 400 }
      );
    }
    ...
    if (!companyName || !taxId || !hourlyRate || !street || !city || !postalCode) {
      return NextResponse.json(
    ...
    if (businessLicenseFile) {
      businessLicenseUrl = await saveUploadedFile(businessLicenseFile, 'businessLicense', session.id);
    } else {
      return NextResponse.json(
    ...
    if (isKisteKlarCertified && kisteKlarCertificateFile) {
      kisteKlarCertificateUrl = await saveUploadedFile(kisteKlarCertificateFile, 'kisteKlarCertificate', session.id);
    } else if (isKisteKlarCertified) {
      return NextResponse.json(
        { success: false, message: 'KisteKlar-Zertifikat ist erforderlich, wenn Sie angeben, zertifiziert zu sein' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { 
        success: true, 
        message: 'Firmenprofil erfolgreich erstellt und zur Überprüfung eingereicht',
        company: newCompany
      },
      { status: 201 }
    );
    ...
    return NextResponse.json(
      { success: false, message: 'Serverfehler beim Erstellen des Firmenprofils' },
      { status: 500 }
    );
  }
}