// src/app/(auth)/register/page.jsx (continuing from previous part)
'use client';
...
export default function Register() {
  ...
  const validateForm = () => {
    ...
    } else if (!/^\+?[0-9\s]{8,}$/.test(formData.phone.replace(/[-()/]/g, ''))) {
      newErrors.phone = 'Bitte geben Sie eine gültige Telefonnummer ein';
    }
    
    if (!formData.password) {
      newErrors.password = 'Bitte geben Sie ein Passwort ein';
    ...
    if (!formData.agreeTerms) {
      newErrors.agreeTerms = 'Sie müssen den AGB und der Datenschutzerklärung zustimmen';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Entferne confirmPassword und agreeTerms aus den Daten, die an den Server gesendet werden
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <div className="mb-6">
            <span className="block text-sm font-medium text-gray-700 mb-2">Ich registriere mich als:</span>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-gray-700">
                {formData.role === 'company' ? 'Firmenname' : 'Name'}
              </label>
              <div className="mt-1">
                <input
                  id="name"
                  name="name"
                  type="text"
                  autoComplete={formData.role === 'company' ? 'organization' : 'name'}
                  value={formData.name}
                  onChange={handleChange}
                  className={`appearance-none block w-full px-3 py-2 border rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm ${
                    errors.name ? 'border-red-300' : 'border-gray-300'
                  }`}
                  placeholder={formData.role === 'company' ? 'Musterfirma GmbH' : 'Max Mustermann'}
                />
                {errors.name && (
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 text-sm">
                <label htmlFor="agreeTerms" className="font-medium text-gray-700">
                  Ich stimme den{' '}
                  <Link href="/agb" className="text-blue-600 hover:text-blue-500">
                    AGB
                  </Link>{' '}
                  und der{' '}
