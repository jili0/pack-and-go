...
export default function CreateOrder() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!submissionData.preferredDates || submissionData.preferredDates.filter((date) => date).length === 0) {
    ...
    try {
      ...
      const calculatedPrice = hourlyRate * submissionData.helpersCount * submissionData.estimatedHours;
          ...
          {[0, 1, 2].map((index) => (
            <label key={index}>
              ...
              <input
                ...
                onChange={(e) => {
                  const newDates = [...(formData.preferredDates || getDefaultDates())];
